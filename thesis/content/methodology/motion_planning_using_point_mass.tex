...
% To effectively model and control vehicle motion, we follow a structured approach consisting of the following key components:

% \begin{itemize}
% 	\item \textbf{Coordinate Transformation and Kinematics:}
% 	      We define the curvature \( C(s) \) of the reference path along with the alignment error \(\xi = \psi - \theta\).
% 	      Using these definitions, we derive the first- and second-order kinematic equations that relate the vehicle's body-fixed velocity components to the
% 	      changes in the Frenet coordinates.

% 	\item \textbf{System Dynamics Formulation:}
% 	      The state vector and control inputs are defined in the Frenet frame.
% 	      We formulate the motion dynamics for the double integrator model, incorporating the influence of curvature and alignment error to capture both
% 	      longitudinal and lateral dynamics along the road.

% 	\item \textbf{Feedback Linearization and System Linearization:}
% 	      To simplify the nonlinear dynamics induced by curvature terms, we introduce an alignment assumption and perform feedback linearization.
% 	      This decouples the system inputs and transforms the model into an affine representation.

% 	\item \textbf{Constraint Handling:}
% 	      We detail the derivation of vehicle and road constraints by mapping bounds on velocity and acceleration from the body-fixed frame to the Frenet
% 	      frame.
% 	      This step is essential for ensuring that the planned trajectories adhere to physical and operational limits.

% 	\item \textbf{Convex Approximation via Quantifier Elimination:}
% 	      Since curvature-dependent constraints introduce non-convexities, we employ quantifier elimination techniques to derive convex inner approximations of
% 	      the feasible set.
% 	      Two methods are presented:
% 	      \begin{itemize}
% 		      \item \emph{Interval Fitting:}
% 		            An approach that provides box-shaped approximations of the constraint set.
% 		      \item \emph{Cylindrical Algebraic Decomposition (CAD):}
% 		            Cylindrical Algebraic Decomposition (CAD) is a computational method used in computer algebra to decompose space into cylindrical cells, allowing for
% 		            the elimination of quantifiers in polynomial constraints while preserving logical equivalence.
% 	      \end{itemize}

% 	\item \textbf{Control Inputs and Steering Angle Derivation:}
% 	      We derive a transformation that maps the optimized state and control variables in the planning model to a steering angle and longitudinal
% 	      acceleration, making the model applicable to standard vehicle control systems.

% 	\item \textbf{Final Model Representation:}
% 	      The complete motion planning model is summarized as a formal tuple that encapsulates the state-space representation, control inputs, system dynamics,
% 	      the convex constraint polytope, and the control transformation.

% \end{itemize}

% With this structured approach, we ensure that motion planning within the Frenet frame is both theoretically sound and practically implementable.
% The following sections present each of these components in detail.
